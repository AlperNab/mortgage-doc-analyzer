...
**Domain:** `Finance / Mortgage` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Mortgage Doc Analyzer** is a standalone, browser-based AI workflow app for **Finance / Mortgage**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `mortgage_documents`<br>Mortgage documents | text | Yes | — | Affects input: Mortgage documents. |
...
| `loan_type`<br>loan type | text | No | — | Affects customization: loan type. |
| `borrower_profile`<br>borrower profile | select | No | — | Affects customization: borrower profile. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `term`<br>term | text | No | — | Affects customization: term. |
| `rate_scenarios`<br>rate scenarios | text | No | — | Affects customization: rate scenarios. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9138
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9138`. |
...
- Export download
- Job history
- Rate/credit bureau systems optional
...
```bash
curl http://127.0.0.1:9138/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9138/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9138 uvicorn app.main:app --host 0.0.0.0 --port 9138
```
...
- `plugins/mortgage-doc-analyzer.json` for plugin metadata.
